...
This file replaces the large heredoc inside run.sh.
...
def _run_with_sysargv(argv: list[str]):
    ...
    for name in names:
        ...
        if strategy == 'vote':
            ...
            # NOTE: when only 1 seed exists, using wsum/wtot cancels confidence.
            if avail == 1 and has_conf_any:
                soft = np.clip(wsum, 0.0, 1.0)
            else:
                soft = wsum / (wtot + 1e-6)
...
def main():
    ...
    argv = ["merge_seeds.py", args.out_multi, args.split, args.clip, str(args.size), args.frame_dir, str(args.minpix)] + list(args.roots)
